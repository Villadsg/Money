...
class StockAnalyzer:
    ...
    def calculate_reaction_duration(self, current_open, current_close, prev_close):
        """Calculate the duration of reaction based on price movement from open to close
        
        Returns:
            Duration in percentage points that the reaction sustained
        """
        # Calculate the reaction strength as the movement from open to close
        reaction_strength = abs((current_close - current_open) / current_open * 100)
        
        # Return the reaction strength as our duration measure
    ...
    def identify_earnings_dates(self, df, threshold_percentile=95, target_dates=15, use_residuals=True):
        ...
        if use_residuals and 'residual_return' in df.columns:
            ...
            # Use the price gap between previous day's close and event day's open
    ...
    def classify_earnings_reactions(self, df):
        """Classify earnings reactions based on price movements and calculate duration-based strength"""
        df = df.copy()
        ...
        for date, row in earnings_dates.iterrows():
            ...
            # Calculate duration-based strength: use the movement from open to close as duration indicator
            duration_strength = self.calculate_reaction_duration(current_open, current_close, prev_close)
            
            classifications.append(classification)
            strengths.append(duration_strength)
